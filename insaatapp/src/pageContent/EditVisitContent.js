...
function EditVisitContent() {
    const [getDtVisitDate, setDtVisitDate] = useState();
    ...
    const btnBack = async () => {
        navigate("/listziyaret");
    };

    const btnSubmit = async () => {
        let requestBody = {
            VisitID: searchParam.get("id"),
            VisitDate: getDtVisitDate,
            Notes: getTxtNotes,
    ...
    useEffect(async () => {
        ...
        setDtVisitDate(document.getElementById("dtVisitDate").value);
        setTxtNotes(document.getElementById("txtNotes").value);
        setCmbCustomer(document.getElementById("cmbCustomer").value);
            ...
            <div className="page-content">
                <div className="container">
                    ...
                    <ul className="page-breadcrumb breadcrumb">
                        <li>
                            <a href="#">Müşteri Yöetimi</a><i className="fa fa-circle" />
                        </li>
                        <li>
                            <Link to="/formvisit">Ziyaret</Link>
                            ...
                            <div className="portlet light">
                                <div className="portlet-title">
                                    <div className="caption">
                                        ...
                                        <span className="caption-subject font-green-sharp bold uppercase">ZİYARETLER</span>
                                    </div>
                                    <div className="tools">
                                    ...
                                    <form role="form">
                                        <div className="form-body">
                                            <div className="form-group">
                                                ...
                                                <input type="date" className="form-control" id="dtVisitDate" name="dtVisitDate" onChange={(e) => setDtVisitDate(e.target.value)}
                                                    defaultValue={visitData
                                                        .filter(
                                                            (visitItem) =>
                                                                visitItem.VisitID == searchParam.get("id")
                                                        )
                                                        .map((arrayItem) => arrayItem.VisitDate)} />
                                            </div>
                                            <div className="form-group">
                                                <label>Müşteri</label>
                                                <select
                                                ...
                                                >
                                                    {customerData.map((customerItem, index) => (
                                                        <option
                                                            key={index}
                                                            name={customerItem.CustomerName}
                                                            value={customerItem.CustomerID}
                                                            selected={
                                                                visitData
                                                                    .filter(
                                                                        (customerItem) =>
                                                                            customerItem.VisitID ==
                                                                            searchParam.get("id")
                                                                    )
                                                                    .map(
                                                                        (arrayItem2) => arrayItem2.CustomerID
                                                                    ) == customerItem.CustomerID
                                                            }
                                                        >
                                                            {customerItem.CustomerName} {customerItem.CustomerSurname}
                                                        </option>
                                                    ))}
                                                </select>
                                            </div>
                                            ...
                                            <div className="form-group">
                                                <label>Notlar</label>
                                                <textarea className="form-control" rows={3} id="txtNotes" name="txtNotes" onChange={(e) => setTxtNotes(e.target.value)}
                                                    defaultValue={visitData
                                                        .filter(
                                                            (visitItem) =>
                                                                visitItem.VisitID == searchParam.get("id")
                                                        )
                                                        .map((myArrayItem) => myArrayItem.Notes)} />
                                            </div>
                                        </div>
...
export default EditVisitContent;